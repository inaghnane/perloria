 ...
 * @access public
 */
class Ecommerce_Comp_Customizer_Range_Control extends WP_Customize_Control {
	...
	public function enqueue() {
		// Main scripts.
		wp_enqueue_script(
			'ecommerce-comp-controls',
			ECOMMERCE_COMP_PLUGIN_URL . '/inc/custom-controls/assets/js/controls.js',
			array(
				'jquery',
				'customize-base',
				'jquery-ui-button',
	...
	/**
	 * Ecommerce_Comp_Customizer_Range_Control constructor.
	 *
 ...
 */
function ecommerce_comp_sanitize_range_value( $input ) {
	if ( ecommerce_comp_is_json( $input ) ) {
 ...
 * @param  string $string Input.
 */
function ecommerce_comp_is_json( $string ) {
	return is_string( $string ) && is_array( json_decode( $string, true ) ) ? true : false;
}
